...
LParser::LParser(const LLexer& lexer, LGlobalData* pGlobalData)
{
	m_pFileOut = NULL;
...
void LParser::re_printf(const char* format, ...)
{
	char buffer[1024];
	va_list args;
	int n;
...
AST* LParser::create_node(AST* node)
{
	if (m_pGlobalData)
...
void LParser::error(const string& err, const LToken* pToken)
{
	char fileinfo[1024];
	char buff[1024];
	if (pToken)
...
void LParser::consume(KEYWORD token_type)
{
...
void LParser::parse_content(const string& content, vector<AST*>& globals, string filename)
{
	LLexer lexer(content, filename);
	LParser parser = LParser(lexer, m_pGlobalData);
		...
		parser.m_current_token.type() == ID)
	{
		...
		if (parser.m_current_token.type() == SHARP)
		{
			...
			if (parser.m_current_token.type() == ID 
				&& (parser.m_current_token.value() == "include"))
			{
...
AST* LParser::class_def(bool bStatic)
{
	global_check_init();
...
AST* LParser::function(string classname, bool bStatic)
{
	global_check_init();
...
AST* LParser::compound_statement()
{
	consume(BEGIN);
	vector<AST*> nodes=statement_list();
	consume(END);
...
bool LParser::CanSkipSEMI(AST* node)
{
...
vector<AST*> LParser::statement_list(string classname)
{
...
AST* LParser::statement(string classname)
{
	AST* node = NULL;
...
AST* LParser::for_statement()
{
	...
	if (m_current_token.type() != SEMI)
		...
		exp = create_node(new Num(LToken(INTEGER, "1", m_current_token.lineNo(), m_current_token.column(), m_current_token.filename())));
	consume(SEMI);
...
AST* LParser::if_statement()
{
	...
	else
	{
		...
		if (m_current_token.type() == SEMI)
		{
			LToken token = m_lexer.peek_next_token();
			if (token.type() == ELSE)
				consume(SEMI);
		}
	}
...
AST* LParser::term_square_dot()
{
        ...
        )
    {
        ...
        else if (m_current_token.type() == DOT)
        {
            LToken token = m_current_token;
            consume(DOT);
            LToken member_token = m_current_token;
            consume(ID);
...
AST* LParser::term_plus_plus()
{
    ...
    if (m_current_token.type() == PLUS_PLUS || m_current_token.type() == MINUS_MINUS)
    {
        ...
        AST* left_child = node;
        AST* right_child = create_node(new Num(LToken(INTEGER, "1",token.lineNo(),token.column(),token.filename())));
        LToken child_token = LToken(PLUS, "+", token.lineNo(), token.column(), token.filename());
        if (token.type() == MINUS_MINUS)
            child_token = LToken(MINUS, "-", token.lineNo(), token.column(), token.filename());
        AST* right = create_node(new BinOp(left_child, child_token, right_child));
        node = create_node(new Assign(node, LToken(ASSIGN, "=", token.lineNo(), token.column(), token.filename()), right));
        ...
        )
    {
		...
		//乘且赋值
            ...
            child_token = LToken(MUL, "*", token.lineNo(), token.column(), token.filename());
		//除且赋值
        else if (token.type() == DIV_EQUAL)
            child_token = LToken(DIV, "/", token.lineNo(), token.column(), token.filename());
		//模且赋值
        else if (token.type() == MOD_EQUAL)
            child_token = LToken(MOD, "%", token.lineNo(), token.column(), token.filename());
		//按位与且赋值
		else if (token.type() == BITWISE_AND_EQUAL)
			child_token = LToken(BITWISE_AND, "&", token.lineNo(), token.column(), token.filename());
		//按位或且赋值
		else if (token.type() == BITWISE_OR_EQUAL)
			child_token = LToken(BITWISE_OR, "|", token.lineNo(), token.column(), token.filename());
		//按位异或且赋值
		else if (token.type() == BITWISE_XOR_EQUAL)
			child_token = LToken(BITWISE_XOR, "^", token.lineNo(), token.column(), token.filename());
		//按位取反且赋值
		else if (token.type() == BITWISE_NOT_EQUAL)
			child_token = LToken(BITWISE_NOT, "~", token.lineNo(), token.column(), token.filename());
		//左移且赋值
		else if (token.type() == LEFT_SHIFT_EQUAL)
		{
			printf("LEFT_SHIFT_EQUAL\n");
			child_token = LToken(LEFT_SHIFT, "<<", token.lineNo(), token.column(), token.filename());
		}
		//右移且赋值
		else if (token.type() == RIGHT_SHIFT_EQUAL)
		{
			printf("RIGHT_SHIFT_EQUAL\n");
			child_token = LToken(RIGHT_SHIFT, ">>", token.lineNo(), token.column(), token.filename());
		}
...
AST* LParser::expr()
{
	LToken prev_token = m_current_token;
...
AST* LParser::array_dict(const LToken& prev_token)
{
	if (m_current_token.type() == LSQUARE)
	{
		consume(LSQUARE);
...
AST* LParser::assignment_array_dict(const LToken& prev_token, const LToken& token, AST* left)
{
...
void  LParser::global_check_init()
{
	m_global_check.push();
}
void  LParser::global_check_start(const vector<AST*>& nodes)
{
